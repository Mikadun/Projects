class Table
{
  ...
  void draw_diagram()
  {
    ...
    for( int i = 0; i < sort_count; i++ )
    {
      ...
      for( int j = 0; j < test_count; j++ )
      {
        System.out.printf("%d %d", times[i][j], j);
...
void draw_diagram()
{  
  int max_time = (int) Math.pow(test_count * 2, test_count / 2);
  ...
  for( int j = 0; j < sort_count; j++ )
  {
    ...
    {
      int x = (int) map( times[j][i], 0, max_time, border, width - border );
      int y = grid_pix_size - i * cell_size;
      ellipse( x, y, 10, 10 );
